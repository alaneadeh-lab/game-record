...
const addCorsHeaders = (req: express.Request, res: express.Response) => {
  ...
  if (origin) {
    ...
    if (
      !productionUrl || // Development mode - allow all
...
app.get('/api/app-data/diagnostic', async (req, res) => {
  try {
    ...
    const allDocsAnalysis = allDocs.map((doc: any) => {
      const data = doc.data || {};
      const sets = data.sets || doc.sets || [];
      const totalGameEntries = sets.reduce((sum: number, set: any) => 
      ...
      const foundCandidateKeys: any = {};
      for (const key of candidateKeys) {
        // Check in sets
        const inSets = sets.reduce((sum: number, set: any) => {
          if (set && key in set && Array.isArray(set[key])) {
            return sum + set[key].length;
          }
          return sum;
...
app.put('/api/app-data', async (req, res) => {
  try {
    ...
    if (existingData) {
      ...
      let mergedSets = incomingData.sets.map((incomingSet: any) => {
      ...
      if (existingData.sets) {
        existingData.sets.forEach((existingSet: any) => {
          if (!mergedSets.some((ms: any) => ms.id === existingSet.id)) {
      ...
      mergedSets = mergedSets.filter((set: any) => !mergedDeletedSetIds.includes(set.id));
      const setsAfterFilter = mergedSets.length;
      
      if (setsBeforeFilter > setsAfterFilter) {
        const removedSetIds = mergedSets
          .map((s: any) => s.id)
          .filter((id: string) => mergedDeletedSetIds.includes(id));
      ...
      const filteredSets = incomingData.sets.filter((set: any) => !mergedDeletedSetIds.includes(set.id));
      mergedData = {
        ...incomingData,
        sets: filteredSets,
        deletedSetIds: mergedDeletedSetIds,
...
function mergeAppData(liveData: AppData | null, restoreData: AppData): AppData {
  if (!liveData) {
    return restoreData;
  }
  ...
  const mergedSets = restoreData.sets.map((restoreSet: any) => {
    const liveSet = liveData.sets?.find((s: any) => s.id === restoreSet.id);
  ...
  if (liveData.sets) {
    liveData.sets.forEach((liveSet: any) => {
      if (!mergedSets.some((ms: any) => ms.id === liveSet.id)) {
...
app.post('/api/app-data/recover', async (req, res) => {
  try {
    ...
    const restoreTotalGameEntries = restoreData.sets?.reduce((sum: number, set: any) => 
      sum + (Array.isArray(set.gameEntries) ? set.gameEntries.length : 0), 0) || 0;
    ...
    const liveTotalGameEntries = liveData?.sets?.reduce((sum: number, set: any) => 
      sum + (Array.isArray(set.gameEntries) ? set.gameEntries.length : 0), 0) || 0;
