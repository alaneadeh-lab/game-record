    ...
    try {
        ...
        const diagnostic = {
            ...
            willBlockBlankOverwrite: foundDoc ? (() => {
                const existingTotal = (foundDoc.data?.sets || foundDoc.sets || []).reduce((sum, set) => sum + (Array.isArray(set.gameEntries) ? set.gameEntries.length : 0), 0);
            ...
            mergedOutcomePreview: foundDoc ? (() => {
                ...
                const existingTotal = existingSets.reduce((sum, set) => sum + (Array.isArray(set.gameEntries) ? set.gameEntries.length : 0), 0);
                return {
...
app.put('/api/app-data', async (req, res) => {
    try {
        ...
        if (incomingDataVersion < existingDataVersion) {
            ...
            const existingTotalGameEntries = existingData?.sets?.reduce((sum, set) => sum + (Array.isArray(set.gameEntries) ? set.gameEntries.length : 0), 0) || 0;
        ...
        const allPlayersWithZeros = incomingData.allPlayers.filter((p) => p.points === 0 && p.fatts === 0 && p.goldMedals === 0 && p.silverMedals === 0 && p.bronzeMedals === 0).length;
...
function checkRecoveryKey(req, res) {
    const providedKey = req.headers['x-recovery-key'];
    if (!RECOVERY_KEY) {
